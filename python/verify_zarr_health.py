...
def check_float16_limit(arr: np.ndarray, var_name: str) -> List[str]:
    ...
    if arr.dtype == np.float16:
        ...
        if max_val > 60000:
            issues.append(
                f"WARNING: {var_name} has values near float16 limit ({max_val:.1f} > 60000)"
            )
        if max_val >= 65504:
            issues.append(
                f"CRITICAL: {var_name} has values at/above float16 limit ({max_val:.1f} >= 65504)"
            )
    return issues
...
def verify_variable_health(
    var_name: str, var: xr.DataArray, sample_rate: float = 1.0
) -> dict:
    """
    ...
    try:
        ...
        if is_numeric:
            ...
            if stats["inf_pct"] > 0:
                stats["issues"].append(
                    f"ERROR: {var_name} contains Infinity values ({stats['inf_pct']:.2f}%)"
                )
        else:
            stats["nan_pct"] = 0.0
            stats["inf_pct"] = 0.0
            logger.debug(
                f"  {var_name}: skipping numeric checks for non-numeric dtype {data.dtype}"
            )

    except Exception as e:
        stats["status"] = "CORRUPT"
        stats["issues"].append(f"CRITICAL: Failed to read {var_name} data: {e!s}")
...
def verify_zarr_store(
    zarr_path: str, sample_rate: float = 1.0, check_chunks: bool = True
) -> bool:
    """
    ...
    try:
        ...
        if is_consolidated:
            ...
            try:
                ...
                logger.warning(
                    "Opened successfully without consolidation. Consolidated metadata might be corrupt."
                )
...
def main():
    ...
    success = verify_zarr_store(
        args.path, sample_rate=sample_rate, check_chunks=check_chunks
    )

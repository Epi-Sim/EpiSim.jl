...
def build_edar_metapop_mapping(edar_ids: set, metapop_df: pd.DataFrame) -> dict:
    ...
    for edar_id in sorted(edar_ids):
        ...
        multi_part_ids = [
            mid
            for mid in metapop_ids
            if mid.startswith(edar_id) and len(mid) == len(edar_id) + 2
        ]
        if multi_part_ids:
...
def create_filtered_metapop_data(
    mapping: dict, metapop_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    for _edar_id, metapop_ids in mapping["multi_part"].items():
...
def create_filtered_mobility_matrix(
    ...
    for edar_id in edar_ids:
        ...
        source_indices = [
            metapop_to_idx[mid] for mid in metapop_ids if mid in metapop_to_idx
        ]
        ...
        for source_idx in source_indices:
            ...
            for _, row in source_rows.iterrows():
                ...
                target_metapop_id = rosetta_df[rosetta_df["idx"] == target_idx][
                    "id"
                ].values
    ...
    result_df = result_df.groupby(["source_idx", "target_idx"], as_index=False)[
        "ratio"
    ].sum()
...
def create_rosetta_edar159(mapping: dict) -> pd.DataFrame:
    ...
    edar_ids = sorted(
        list(mapping["exact_match"].keys())
        + list(mapping["with_suffix"].keys())
        + list(mapping["multi_part"].keys())
    )

    return pd.DataFrame({"id": edar_ids, "idx": range(1, len(edar_ids) + 1)})
...
def main():
    ...
    print(
        f"Saved {len(filtered_mobility_df)} mobility entries to {mobility_output_path}"
    )
    ...
    print(
        f"Saved rosetta with {len(rosetta_edar159_df)} entries to {rosetta_output_path}"
    )
    ...
    assert filtered_mobility_df["source_idx"].max() <= max_idx, (
        "Invalid source_idx in mobility"
    )
    assert filtered_mobility_df["target_idx"].max() <= max_idx, (
        "Invalid target_idx in mobility"
    )

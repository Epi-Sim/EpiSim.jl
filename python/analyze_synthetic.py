...
if "synthetic_strength" in ds and intervention_mask.any():
    ...
    print(
        f"Min: {np.min(strengths):.2f}, Max: {np.max(strengths):.2f}, Mean: {np.mean(strengths):.2f}"
    )
    print()
...
for scenario_type in np.unique(scenarios):
    ...
    if "infections_true" in subset:
        ...
        hosp = (
            subset["hospitalizations_true"]
            .sum(dim=["run_id", "region_id", "date"])
            .values
        )
...
if intervention_mask.any():
    ...
    if effectiveness_data:
        ...
        for scenario_type in df_eff["scenario_type"].unique():
            ...
            print(
                f"  Range: [{subset['effectiveness'].min():.1%}, {subset['effectiveness'].max():.1%}]"
            )
            print()

        print("=== Effectiveness by Strength ===")
        strength_groups = df_eff.groupby("strength")["effectiveness"].agg(
            ["mean", "std", "min", "max"]
        )
    ...
    if baseline_stats:
        ...
        print(
            f"Mean Infections:       {df_baseline['infections'].mean():,.0f} (±{df_baseline['infections'].std():,.0f})"
        )
        print(
            f"Mean Hospitalizations: {df_baseline['hospitalizations'].mean():,.0f} (±{df_baseline['hospitalizations'].std():,.0f})"
        )
        print(
            f"Mean Deaths:           {df_baseline['deaths'].mean():,.0f} (±{df_baseline['deaths'].std():,.0f})"
        )
        print()

        print("=== Range Across Baselines ===")
        print(
            f"Infections:       [{df_baseline['infections'].min():,.0f}, {df_baseline['infections'].max():,.0f}]"
        )
        print(
            f"Hospitalizations: [{df_baseline['hospitalizations'].min():,.0f}, {df_baseline['hospitalizations'].max():,.0f}]"
        )
        print(
            f"Deaths:           [{df_baseline['deaths'].min():,.0f}, {df_baseline['deaths'].max():,.0f}]"
        )
        print()
...
if "infections_true" in baseline_ds:
    ...
    daily_inf_baseline = (
        baseline_ds["infections_true"].sum(dim=["run_id", "region_id"]).values
    )
    ...
    if intervention_mask.any():
        ...
        for i, profile in enumerate(profiles_to_plot):
            ...
            if "baseline" in scenarios_dict:
                ...
                baseline_inf = (
                    ds["infections_true"]
                    .isel(run_id=baseline_idx)
                    .sum(dim="region_id")
                    .values
                )
                dates = pd.to_datetime(ds.date.values)
                ax.plot(
                    dates, baseline_inf, label="Baseline", linewidth=2, color="black"
                )
            ...
            for scenario, run_id in scenarios_dict.items():
                if scenario != "baseline":
                    ...
                    ax.plot(
                        dates, scenario_inf, label=label, alpha=alpha, linewidth=1.5
                    )
        ...
        run_ids_sorted = sorted(
            ds.run_id.values, key=lambda x: int(x.strip().split("_")[0])
        )

        # Plot each baseline
        for run_id in run_ids_sorted[:10]:  # Plot first 10
            idx = np.where([x.strip() == run_id.strip() for x in ds.run_id.values])[0][
                0
            ]
            infections = (
                ds["infections_true"].isel(run_id=idx).sum(dim="region_id").values
            )
            dates = pd.to_datetime(ds.date.values)
            label = run_id.strip()

...
def main():
    ...
    if len(unmatched) > 0:
        print(
            f"  Warning: {len(unmatched)} census municipalities not found in reference:"
        )
    ...
    census_pivot = census_2024.pivot_table(
        index="muni_code", columns="age", values="value", aggfunc="sum"
    ).reset_index()
    ...
    rosetta_df = pd.DataFrame(
        {"id": metapop_df["id"], "idx": range(1, len(metapop_df) + 1)}
    )

    # Create ID to index mapping
    id_to_idx = dict(zip(rosetta_df["id"], rosetta_df["idx"]))
    dict(zip(rosetta_df["idx"], rosetta_df["id"]))
        ...
        for j in range(n_muni):
            ...
            if ratio > threshold:
                ...
                mobility_rows.append(
                    {"source_idx": source_idx, "target_idx": target_idx, "ratio": ratio}
                )
    ...
    config["data"] = {
        ...
        "kappa0_filename": config["data"].get(
    ...
    print(
        f"  Population: Y={summary['population_summary']['total_Y']:,}, "
        f"M={summary['population_summary']['total_M']:,}, "
        f"O={summary['population_summary']['total_O']:,}"
    )
    print(
        f"  EDAR municipalities: {summary['n_edar_in_dataset']}/{summary['n_edar_municipalities']}"
    )
    print("\nOutput files:")

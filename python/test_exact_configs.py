...
def run_scenario(name, kappa_s, tc_s, kappa0_csv=None):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"{name:30s} κ₀={kappa_s} tᶜ={tc_s:3d}: {S_final:12.0f} susceptibles")

print("Testing exact configurations:")
print("-" * 70)

# Test 1: Baseline config (κ₀=[1.0], tᶜ=[0])
run_scenario("Baseline (κ₀=1.0, tᶜ=0)", [1.0], [0])

# Test 2: Intervention config (κ₀=[1.0], tᶜ=[0]) with CSV κ₀=0.2
# Create CSV
import pandas as pd
with tempfile.TemporaryDirectory() as temp_dir:
...
# Test 3: No intervention (κ₀=[0.0], tᶜ=[0])
run_scenario("No κ₀ value (κ₀=0.0, tᶜ=0)", [0.0], [0])

# Test 4: Default from base config (κ₀=[0.8], tᶜ=[100])

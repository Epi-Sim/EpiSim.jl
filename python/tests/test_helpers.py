...
class TestHelpers:
    ...
    def create_mock_success_result(
        output: str = "Simulation completed successfully",
    ) -> MockResult:
    ...
    def create_mock_failure_result(
        error_msg: str = "Simulation failed", output: str = "Some output"
    ) -> MockResult:
    ...
    def create_minimal_config(group_size: int = 3) -> Dict[str, Any]:
        ...
        group_labels = (
            ["Y", "M", "O"][:group_size]
            if group_size <= 3
            else [f"G{i}" for i in range(group_size)]
        )
        ...
        gamma_g = [
            0.003 if i == 0 else (0.01 if i == 1 else 0.08) for i in range(group_size)
        ]
        zeta_g = [0.13] * group_size
        lambda_g = [1.0] * group_size
        omega_g = [
            0.0 if i == 0 else (0.04 if i == 1 else 0.3) for i in range(group_size)
        ]
        ...
        contact_matrix = [
            [0.6 if i == j else 0.4 / (group_size - 1) for j in range(group_size)]
    ...
    def create_test_rosetta_content(
        ...
        higher_regions: Optional[List[str]] = None,
    ) -> str:
    ...
    def setup_episim_model(
        config: Dict[str, Any],
        temp_dir: str,
        instance_folder: Optional[str] = None,
    ...
    @staticmethod
    def assert_episim_model_structure(
        model: EpiSim, expected_engine: str = "MMCACovid19Vac"
    ):
    ...
    def assert_config_structure(
        config: EpiSimConfig, expected_groups: Optional[List[str]] = None
    ):
        ...
        assert config.config["simulation"]["engine"] in [
            "MMCACovid19",
            "MMCACovid19Vac",
        ]

    @staticmethod
    def create_invalid_config_missing_section(
        base_config: Dict[str, Any], section: str
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def create_invalid_config_missing_key(
        base_config: Dict[str, Any], section: str, key: str
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def create_invalid_config_wrong_group_size(
        base_config: Dict[str, Any], param: str, wrong_size: int
    ) -> Dict[str, Any]:
    ...
    def create_test_xarray_data(
        ...
        age_groups: Optional[List[str]] = None,
        time_steps: int = 5,
        epi_states: Optional[List[str]] = None,
    ) -> xr.DataArray:
        ...
        data = np.random.rand(
            len(age_groups), len(regions), time_steps, len(epi_states)
        )
        coords = {
            "G": age_groups,
            "M": regions,
...
class AssertionHelpers:
    ...
    @staticmethod
    def assert_config_saved_correctly(
        config_path: str, expected_values: Dict[str, Any]
    ):
        ...
        for key, expected_value in expected_values.items():
            ...
            for k in keys[:-1]:
                current = current[k]
            assert current[keys[-1]] == expected_value, (
                f"Config key {key} should be {expected_value}"
            )

    @staticmethod
    def assert_subprocess_called_correctly(
        mock_run, expected_args: List[str], expected_in_args: Optional[List[str]] = None
    ):
        ...
        for expected_arg in expected_args:
            assert expected_arg in args, (
                f"Expected argument {expected_arg} not found in {args}"
            )

        if expected_in_args:
            for expected_in_arg in expected_in_args:
                assert any(expected_in_arg in arg for arg in args), (
                    f"Expected substring {expected_in_arg} not found in args"
                )

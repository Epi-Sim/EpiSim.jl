...
- **Mobility noise**: Time-varying mobility can be generated by sampling origin/destination noise
  (mobility sigma). Use `--mobility-sigma-min` / `--mobility-sigma-max` when running
  `python/run_synthetic_pipeline.py` or `python/synthetic_generator.py`.
